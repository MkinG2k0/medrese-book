...
function getCellLabel(request: LeaveRequestListItem, mode: LeaveCalendarMode) {
  if (mode === "manager") {
    return abbreviateTeacherSurname(request.teacherName);
  }
  return `${getLeaveRequestTypeLabel(request.type)}`;
}
...
export function LeaveCalendar({
  ...
  const cellRender = (current: Dayjs, info: { type: string }) => {
    ...
    return (
      <div className="flex min-w-20 flex-col gap-0.5 pt-1">
        {visible.map((request) => (
          <button
  ...
  return (
    <div className="flex flex-col gap-4">
      <Calendar
        fullscreen={false}
        cellRender={(current, info) => cellRender(current as Dayjs, info)}
        onSelect={(date) => {
